...
- **No page-load choreography, with four exceptions.** The rule exists because animating a screen somebody is *working* on puts a delay between them and the thing they came to do, so the test for an exception is whether there is a task on the surface at all.
  ...
  - The 404 (the `.nf` section at the end of `globals.css`). A dead end is a full stop, not a task: there is one line to read and three ways out, so there is nothing for the motion to sit in front of. One curve, one distance, one interval, so six elements arrive as a single wave rather than six animations.
